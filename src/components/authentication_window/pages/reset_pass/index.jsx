        ...
        <div className="reset-password">
            ...
            <form action="" onSubmit={handleSubmit} >
                <fieldset disabled={loading}>
                    <InputBox placeholder="Enter new password" name="password" input_type="password" error={error.type === "password" && error.message} />
                    <p className="form-error-msg">
                        {error.message && (error.type !== "password") && 
                        <span className="error-text">{error.message}</span>}
                    </p>
                    { !ifResetSuccess ?
                        <SubmitBtn /> : <div className="btn-done" onClick={() => changePage("login", true)}>Done</div> }
                </fieldset>
            </form>
